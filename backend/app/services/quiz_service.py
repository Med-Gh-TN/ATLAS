...
from app.services.generation_service import generate_feedback_for_missed_question
...
class QuizEvaluationEngine:
    ...
    async def _generate_targeted_feedback(self, question: Question, student_answer: str) -> str:
        ...
        try:
            ...
            logger.error(f"Failed to generate AI feedback for question {question.id}: {str(e)}")
    ...
    async def evaluate_and_store(self, user_id: str, payload: QuizSubmitPayload) -> QuizEvaluationResult:
        ...
        for ans in payload.answers:
            ...
            is_correct = ans.student_answer.strip().lower() == q_entity.correct_answer.strip().lower()
            ...
            if is_correct:
                ...
                feedbacks.append(QuestionFeedback(
                    ...
                    is_correct=True,
                    correct_answer=q_entity.correct_answer,
                    student_answer=ans.student_answer,
                    source_page=str(getattr(q_entity, 'source_page', ''))
                ))
            else:
                ...
                task = self._generate_targeted_feedback(q_entity, ans.student_answer)
                feedback_tasks.append((feedback_item, q_entity, task))

        if feedback_tasks:
            items, q_entities, awaitables = zip(*feedback_tasks)
            ai_responses = await asyncio.gather(*awaitables)
