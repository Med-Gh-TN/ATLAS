    ...
    async def check_and_register_active_session(redis_client: Any, student_id: uuid.UUID, session_id: uuid.UUID) -> None:
        ...
        try:
            ...
            if count >= RAGRateLimits.MAX_ACTIVE_SESSIONS:
                logger.warning(f"RAG Limit Exceeded: Student {student_id} attempted to open >{RAGRateLimits.MAX_ACTIVE_SESSIONS} sessions.")
                raise HTTPException(
                    status_code=status.HTTP_429_TOO_MANY_REQUESTS,
                    detail=f"Maximum of {RAGRateLimits.MAX_ACTIVE_SESSIONS} active RAG sessions reached. Please close an existing session."
                )
