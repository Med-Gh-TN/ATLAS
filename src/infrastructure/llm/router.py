"""
@file router.py
@description Omni-Architect: Model Routing & Resilience Layer (v8.2 Token Safe Edition)
────────────────────────────────────────────────────────────────────────────────
...
Changelog v8.2:
  - SOTA FIX: Honor explicit max_token overrides from the bridge to prevent 
    the Sovereign Node from truncating massive AST JSON payloads.
"""
...
class TaskRouter:
    ...
    async def _call_single_model(
        ...
        elif task and task.value == "ASSET_GENERATION":
    ...
    async def route_call(
        ...
        **kwargs
        ...
        # SOTA FIX: Copy the chain to avoid mutating the global registry
        model_chain = list(self.registry.get_chain(task))
        last_exception: Optional[Exception] = None

        # ── SOTA FIX: Sovereign Edge Dynamic Prioritization ──
        use_external = os.getenv("USE_EXTERNAL_GPU", "false").lower() == "true"
        colab_url = os.getenv("COLAB_GPU_URL")
        
        if not (use_external and colab_url):
            if VLLM_SENTINEL in model_chain:
                model_chain.remove(VLLM_SENTINEL)
        else:
            if VLLM_SENTINEL in model_chain:
                model_chain.remove(VLLM_SENTINEL)
        ...
        for model_id in model_chain:
            ...
            # ── 1. SOVEREIGN NODE INTERCEPT (vLLM via Tunnel) ──
            if model_id == VLLM_SENTINEL:
                cb_state = await self.circuit_breaker.get_state(task, model_id)
                if cb_state == CBState.OPEN:
                    logger.info("route_call [%s]: Sovereign Edge CB is OPEN — skipping to Google fallback.", task.value)
                    continue

                logger.info("route_call [%s]: Routing to Sovereign Edge Node (Qwen3-VL).", task.value)
                try:
                    if max_tokens_override is not None:
                        max_tokens = max_tokens_override
                    elif task == TaskType.ASSET_GENERATION:
                        max_tokens = int(os.getenv("GEMINI_MAX_ASSET_TOKENS", "8192"))
                    else:
                        max_tokens = self.registry.output_budget(VLLM_SENTINEL, extraction_mode)
                    ...
                    logger.warning(
                        "route_call [%s]: Sovereign Node failed (%s). Falling back to Gemma cascade.", 
                        task.value, exc
                    )
                    continue

            # ── 2. STANDARD GOOGLE AI STUDIO ROUTING ──
            cb_state = await self.circuit_breaker.get_state(task, model_id)
            if cb_state == CBState.OPEN:
        ...
        raise RuntimeError(
            f"route_call: exhausted ALL models in the fallback chain for task {task.value}. "
